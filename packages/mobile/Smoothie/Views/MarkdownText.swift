...
/// Renders agent message content as markdown. Splits the input into blocks
/// (paragraph, heading, list, fenced code, blockquote, hr) and lays them out in
/// a `VStack`. Inline elements (`**bold**`, `*italic*`, `` `code` ``, `[link](…)`)
/// inside text blocks are parsed via Foundation's `AttributedString(markdown:)`.
struct MarkdownText: View {
    ...
    private var blocks: [Block] {
        ...
        if inFence, !fence.isEmpty {
            // Unclosed fence — emit as code anyway
            out.append(.codeBlock(fence.joined(separator: "\n"), fenceLang))
        }
        flushAllExcept()
...
#Preview {
    ZStack {
        ...
        ScrollView {
            ...
            Regular paragraph with **bold**, *italic*, and `inline code`. Also a [link](https://example.com).
            ...
            > A blockquote with *flair*.

            ```swift
            func hello() {
                print("hi from Smoothie")
            }
            ```

            ---

            Trailing paragraph after rule.
            """)
            .padding(16)
        }
    }
